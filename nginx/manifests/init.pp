...
exec { "./configure in /tmp/nginx-1.4.4":
        command => "/tmp/nginx-1.4.4/configure --without-mail_pop3_module --user='nginx' --sbin-path=/usr/local/nginx --conf-path=/usr/local/cluster/nginx/nginx.conf --group='nginx' --with-http_ssl_module --without-mail_imap_module --without-mail_smtp_module --lock-path=/var/lock/nginx.lock --pid-path=/var/run/nginx.pid --with-http_stub_status_module --with-pcre",
cwd => '/tmp/nginx-1.4.4',
